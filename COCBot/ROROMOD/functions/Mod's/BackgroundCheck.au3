...
EndFunc   ;==>chkPic
 ...
 Func BackGr()
     Local $iKey
            ...
            GUICtrlSetImage(Eval("" & $i), @ScriptDir & '\COCBot\ROROMOD\Images\' & $iKey & '.jpg')
        
		 For $i = 43 To 47
            $iKey = StringRight(GUICtrlRead($BackGr), 1)
            GUICtrlSetImage(Eval("" & $i), @ScriptDir & '\COCBot\ROROMOD\Images2\' & $iKey & '.jpg')
        Next
		 For $i = 49 To 50
            $iKey = StringRight(GUICtrlRead($BackGr), 1)
            GUICtrlSetImage(Eval("" & $i), @ScriptDir & '\COCBot\ROROMOD\Images3\' & $iKey & '.jpg')
        Next
